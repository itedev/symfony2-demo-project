...
class ChoiceListBuilder
{
    ...
    protected static function getChoices(array $choices)
    {
        return array_map(function($number) {
            return str_replace('-', ' ', static::number2Text($number));
        }, $choices);
    }
     ...
     * @return bool|string
     */
    protected static function number2Text($number)
    {
        $hyphen      = '-';
        $conjunction = ' and ';
        $separator   = ', ';
        ...
        if ($number < 0) {
            return $negative . static::number2Text(abs($number));
        }
        ...
        switch (true) {
            ...
            case $number < 1000:
                ...
                if ($remainder) {
                    $string .= $conjunction . static::number2Text($remainder);
                }
                break;
            default:
                ...
                $string = static::number2Text($numBaseUnits) . ' ' . $dictionary[$baseUnit];
                if ($remainder) {
                    $string .= $remainder < 100 ? $conjunction : $separator;
                    $string .= static::number2Text($remainder);
                }
                break;
        }
